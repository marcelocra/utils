# This is a very interesting script that the JetBrains folks provide as the
# install script for their JetBrainsMono font in Linux systems. 
# 
# It can be found on their [JetBrainsMono
# repo](https://github.com/JetBrains/JetBrainsMono) in the link below:
# 
# https://raw.githubusercontent.com/JetBrains/JetBrainsMono/master/install_manual.sh
#
# Here, it was not meant as an executable script, but only as a reference,
# parcicularly because of a couple of interesting stuff that I was not aware of:
# 
# bash
# ----
#
# - a variable can be made `readonly` after being defined
# - there's a simple way of defining colors with `echo`
# - we can create a simple for-each with a list of strings simply separating the
#   elements with a space. the `for ... in` split it correctly, it seems
#
# github
# ------
#
# There's an `api` version of the `releases` page, with `json` output.  This
# makes it simpler to write scripts that read from there.
