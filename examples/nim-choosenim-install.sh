...
#
# This is the recommended way of installing the Nim programming language, from
# their website: https://nim-lang.org/install_unix.html
#
# It has some interesting ideas that I list below and I plan to use. For each
# number, the 'ideaN' string is added to the appropriate place in the script.
#
#   1) Uses each `set` statement in its own line. This is even more helpful when
#   the command is used with its full name, for example:
#
#       set -o errexit (equivalent to set -e)
#       set -o nounset (equivalent to set -u)
#       set -o xtrace (equivalent to set -x)
#
#   2) Defines some variables from an env, but with default values. It is a
#   simple way of allowing users to change stuff.
#
#   3) Keeps less relevant functions below `install`. I didn't know we could do
#   this, as many languages expects symbols to be defined before they can be
#   referenced.
#
#   4) Uses `getopts` to parse command line options. I've never used it and
#   seems like a simple way of doing it.
#
#   5) Stops the script if interaction with the user is needed and it is not
#   available. I didn't know that there's a way to check for that.
#
#   6) It is possible to create two `local` variables with same name, using one
#   in the other... seems very bug prone hahahha, but also useful.
#
#   7) They use this script to download the choosenim binary, make it executable
#   and execute it.
#
#   8) It is possible to use `local` inside a conditional branch. Not sure what
#   the scope will be, though.
#
#   9) Interesting way of naming functions: `say_err` echos stuff while `err`
#   actually causes the script to exit. All of them defined below `install`, see
#   item 3, above.

echo 'Example script not meant to be run!'
exit 1
